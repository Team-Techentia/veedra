...
const EditComboPage = () => {
  ...
  const fetchCombo = async () => {
    try {
      setLoading(true);
      const response = await comboService.getCombo(id);
      console.log('Combo response:', response); // Debug log
      
      // The response is already the combo object (comboService.getCombo returns response.data)
      const combo = response;
      
      // Check if combo data exists
      if (!combo) {
        throw new Error('Combo data not found');
      }
      ...
      setFormData({
        ...
        discountValue: combo.discountValue || 5,
        validFrom: combo.validFrom ? new Date(combo.validFrom).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        validUntil: combo.validUntil ? new Date(combo.validUntil).toISOString().split('T')[0] : '',
        priceSlots: combo.priceSlots?.length > 0 ? combo.priceSlots : [{
          slotName: 'Basic Slot',
          minPrice: 0,
          maxPrice: 500,
          maxProducts: 5,
        ...
        rules: {
          totalProducts: {
            min: combo.rules?.totalProducts?.min || 2,
            max: combo.rules?.totalProducts?.max || 10
          },
          allowDuplicates: combo.rules?.allowDuplicates || false,
          requireAllSlots: combo.rules?.requireAllSlots || false,
          minimumValue: combo.rules?.minimumValue || 0,
          maximumValue: combo.rules?.maximumValue || 999999
        },
        autoAssignment: {
          enabled: combo.autoAssignment?.enabled !== undefined ? combo.autoAssignment.enabled : true,
          preventHighValueInLowSlot: combo.autoAssignment?.preventHighValueInLowSlot !== undefined ? combo.autoAssignment.preventHighValueInLowSlot : true,
          allowPriceAdjustment: combo.autoAssignment?.allowPriceAdjustment || false
        },
        usageLimits: {
          totalUsage: combo.usageLimits?.totalUsage || null
        }
      });
    } catch (error) {
      ...
      // Check if combo doesn't exist
      if (error.response?.status === 404) {
        toast.error('Combo not found');
      } else {
        toast.error('Failed to load combo: ' + (error.response?.data?.message || error.message));
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await comboService.updateCombo(id, comboData);
      toast.success('Combo updated successfully!');
      navigate('/combos');
    } catch (error) {
      console.error('Error updating combo:', error);
      const message = error.response?.data?.message || 'Failed to update combo';
      toast.error(message);
    } finally {
      setSaving(false);
    }
      ...
      <div className="p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-600">Loading combo...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-600">Update combo settings and rules</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end space-x-3">
          ...
          >
            ...
            <span>{saving ? 'Updating...' : 'Update Combo'}</span>
          </button>
        </div>
      </form>
    </div>
