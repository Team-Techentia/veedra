...
const { generateProductCode, generateChildCode } = require('../services/productCodeService');
...
const createProduct = asyncHandler(async (req, res) => {
  try {
    ...
    if (productData.bundle?.isBundle) {
      ...
      if (!productData.inventory?.currentStock || productData.inventory.currentStock < 2) {
        return res.status(400).json({
          status: 'error',
          message: 'Bundle must have at least 2 items (1 parent + 1 child)'
        });
      }
